...
class SiteStatus:
    ...
    error_message: Optional[str] = None
    # Meter data — populated by platforms that expose grid-connection meter readings
    meter_export_kwh: Optional[float] = None  # Energy exported to grid today (kWh)
    meter_import_kwh: Optional[float] = None  # Energy imported from grid today (kWh)
...
class MeterComparisonStatus(Enum):
    """Outcome of comparing inverter generation against meter export data."""
    OK            = "ok"            # Within expected tolerance
    DISCREPANCY   = "discrepancy"   # Loss factor deviates beyond threshold
    INVERTER_ONLY = "inverter_only" # Inverter shows generation, meter shows zero/None
    METER_ONLY    = "meter_only"    # Meter shows export, inverter shows zero
    DATA_MISSING  = "data_missing"  # Meter data not available for this platform/site


@dataclass
class MeterComparisonResult:
    """Result of comparing inverter generation against meter export for one site."""
    site_id: str
    site_name: str
    platform: str
    inverter_kwh: Optional[float]        # Total inverter generation today (kWh)
    meter_export_kwh: Optional[float]    # Grid export from meter today (kWh)
    loss_factor_pct: Optional[float]     # (inverter - meter) / inverter * 100
    discrepancy_pct: Optional[float]     # abs(loss_factor - expected_loss_pct)
