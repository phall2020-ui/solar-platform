...
from components.job_monitor import (
    auto_refresh_on_active_jobs,
    job_status_viewer,
    render_job_status_badge,
)
from services.background_jobs import JobStatus, get_job_manager, submit_background_job
...
def _render_pull_tab():
    ...
    with col1:
        if st.button("🔍 Discover Plants", use_container_width=True, type="primary"):
            from services.juggle_data_pull import bg_discover_plants
    ...
    if pull_clicked:
        from services.juggle_data_pull import bg_pull_all_plants, bg_pull_single_plant
