...
from clients.base import BaseInverterClient
...
def generate_report(all_statuses: List[SiteStatus], logger: logging.Logger) -> tuple[str, list, list]:
    ...
    for platform, items in platforms.items():
        ...
        lines.append(f"{'Site Name':<30} | {'Cap(kWp)':<8} | {'Pwr(kW)':<8} | {'Status':<10} | {'Last Data':<10} |")
        lines.append(f"{'-'*30}-|-{'-'*8}-|-{'-'*8}-|-{'-'*10}-|-{'-'*10}-|")
        ...
        for status, last_contact in items:
            ...
            lines.append(f"{name:<30} | {status.installed_capacity_kw:<8.1f} | {status.total_power_kw:<8.1f} | {status_str:<10} | {last_contact:<10} |")
...
def check_all_inverters(config: dict, logger: logging.Logger, save_to_file: bool = True, skip_state_save: bool = False):
    ...
    report_text, comm_fails, partials = generate_report(all_statuses, logger)
    print(report_text)
