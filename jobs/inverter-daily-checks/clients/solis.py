...
class SolisClient(BaseInverterClient):
    ...
    def __init__(self, config: dict):
        ...
        self.api_url = config.get('api_url', 'https://www.soliscloud.com:13333')
    ...
    def _generate_signature(self, body: str, path: str) -> dict:
        """
        Generate authentication headers with HMAC-SHA1 signature.
        
        The signature is calculated as:
        Sign = Base64(HmacSHA1(apiSecret, "POST\n" + Content-MD5 + "\napplication/json\n" + Date + "\n" + Path))
        """
        # Content-MD5: MD5 hash of body, converted to bytes, then Base64 encoded
        ...
        authorization = f"API {self.api_id}:{signature_b64}"
    ...
    def _api_request(self, path: str, data: dict) -> dict:
        ...
        headers = self._generate_signature(body, path)
