...
def build_email_body(
    ...
    def _alert_label(diff_frac: float) -> Optional[str]:
    ...
    lines.append("")
    lines.append(f"Monthly Solar Portfolio Summary - {month_name} {month_start.year}")
    lines.append("")
    lines.append(
        f"Previous Month Alerts (Warning/Critical) - {month_start.isoformat()} to {month_end.isoformat()}"
    )
    if alert_lines:
    ...
    lines.append(f"- Synced Sites: {synced_sites} / {total_sites}")
