...
def build_snapshot_details(
    *,
    skipped_no_juggle_mapping: list[str],
    skipped_inverter_only: list[str],
    skipped_no_comparison_data: list[str],
...
def main():
    ...
    for name, info in mapping.items():
        ...
        for day_str in target_days:
            ...
            comparison_row = {
                ...
                "date": day_str,
                "platform": platform,
                "juggle_inverter_kwh": round(d_inv, 2),
                "juggle_meter_kwh": round(d_pv, 2),
                "platform_kwh": round(d_platform, 2),
            }
            if str(platform).strip().lower() == "solis":
                comparison_row["solis_kwh"] = round(d_platform, 2)
    ...
    snapshot_path = save_run_snapshot(
        ...
        run_mode,
        target_days,
        details=build_snapshot_details(
            skipped_no_juggle_mapping=skipped_no_juggle_mapping,
            skipped_inverter_only=skipped_inverter_only,
            skipped_no_comparison_data=skipped_no_comparison_data,
            site_errors=site_errors,
